 ...
 * Machine (system) specifications:
    ...
    * main_v4.cu – Optimized ride‑sharing dispatch with:
    *   - Fixed small search radius (MAX_RADIUS = 5)
    *   - Fallback global search for remaining riders
    *   - Driver IDs stored in grid (coordinates remain separate)
 ...
 *************************************************************************************************/
 ...
 __global__ void spatial_grid_partitioning_kernel(
         ...
         int *grid_driver)       // [NUM_CELLS * MAX_CAPACITY]
 {
     int tid = blockIdx.x * blockDim.x + threadIdx.x;
     if (tid >= NUM_DRIVERS) return;
     ...
     if (cell_x >= GRID_DIM) cell_x = GRID_DIM - 1;
     if (cell_y >= GRID_DIM) cell_y = GRID_DIM - 1;
 
     int cell_id = cell_y * GRID_DIM + cell_x;
     int slot = atomicAdd(&grid_count[cell_id], 1);
     if (slot < MAX_CAPACITY) {
         grid_driver[cell_id * MAX_CAPACITY + slot] = tid;
     }
 }
 ...
 __global__ void ride_sharing_kernel(
         ...
         const int *grid_driver)
 {
     int tid = blockIdx.x * blockDim.x + threadIdx.x;
     if (tid >= NUM_RIDERS) return;
     ...
     if (home_cx < 0) home_cx = 0;
     if (home_cx >= GRID_DIM) home_cx = GRID_DIM - 1;
     if (home_cy < 0) home_cy = 0;
     if (home_cy >= GRID_DIM) home_cy = GRID_DIM - 1;
 
     int best_driver = -1;
     int best_dist = INT_MAX;
     bool booked = false;
 
     // Expand radius layer by layer
     for (int r = 0; r <= MAX_RADIUS && !booked; ++r) {
         // Scan only cells at Chebyshev distance exactly r (the perimeter)
         for (int dx = -r; dx <= r; ++dx) {
             for (int dy = -r; dy <= r; ++dy) {
                 if (abs(dx) != r && abs(dy) != r) continue;   // inner cells already handled
                 ...
                 for (int k = 0; k < count; ++k) {
                     ...
                     int dy_coord = ry - driver_y[driver_id];
         ...
         if (best_driver != -1) {
             if (book_driver(best_driver, driver_info)) {
                 ...
                 // The failed driver is now marked as booked, so it will be skipped
                 // in subsequent scans.
                 best_driver = -1;
                 best_dist = INT_MAX;
                 // Continue to next radius (which will include this layer again,
                 // but that's okay with small MAX_RADIUS)
             }
         }
     }
 ...
 __global__ void fallback_global_kernel(
         ...
         int *driver_info,
         int *result,
         const int *failed_indices,   // list of rider IDs that need global search
         int num_failed)
 {
     ...
     while (!booked) {
         ...
         for (int d = 0; d < NUM_DRIVERS; ++d) {
             int idx_bit = d >> 5;
             int bit = d & 31;
             if (driver_info[idx_bit] & (1U << bit)) continue;   // booked
         ...
         // else loop again (the failed driver is now marked, next scan will skip it)
     }
 }
 ...
 int main() {
     ...
     int *d_failed_indices;   // for fallback
     ...
     CHECK(cudaMalloc(&d_failed_indices, NUM_RIDERS * sizeof(int))); // worst case
     ...
     int matched = 0;
     for (int i = 0; i < NUM_RIDERS; ++i) {
         if (h_results[i] != -1) matched++;
     }
     printf("After radius search: %lf %% matched, %lf %% failed\n", (double)matched/NUM_RIDERS * 100, (double)(NUM_RIDERS - matched)/NUM_RIDERS * 100);
 
     // Step 3: Fallback global search for failed riders
     int failed_count = NUM_RIDERS - matched;
     if (failed_count > 0) {
        printf("Launching fallback global search for %lf %% riders...\n", (double)failed_count / NUM_RIDERS * 100);
        // Compact failed rider indices on host
     ...
     // Cleanup
     free(h_rx); free(h_ry); free(h_dx); free(h_dy); free(h_results); free(h_info);
     cudaFree(d_rx); cudaFree(d_ry); cudaFree(d_dx); cudaFree(d_dy);
     cudaFree(d_info); cudaFree(d_res); cudaFree(d_grid_count);
     cudaFree(d_grid_driver); cudaFree(d_failed_indices);
